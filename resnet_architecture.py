...
def inference(x, x_depth, is_training,
              num_classes=1000,
    ...
    with tf.variable_scope('depth_stream'):
        with tf.variable_scope('scale1'):
            c['conv_filters_out'] = 64
            c['ksize'] = 7
            c['stride'] = 2
            x_depth = conv(x_depth, c)
            x_depth = bn(x_depth, c)
            x_depth = activation(x_depth)

        with tf.variable_scope('scale2'):
            x_depth = _max_pool(x_depth, ksize=3, stride=2)
            c['num_blocks'] = num_blocks[0]
            c['stack_stride'] = 1
            c['block_filters_internal'] = 64
            x_depth = stack(x_depth, c)
